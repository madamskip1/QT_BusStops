...
QVariant TreeModel::data(const QModelIndex &index, int role) const
{
    if(index.isValid())
    {
        TreeNode *node = static_cast<TreeNode*>(index.internalPointer());
         ...
         else if (role == StopName)
            return node->data(1).toString();
         else if (role == StopDir)
             return node->data(2).toString();
...
QHash<int, QByteArray> TreeModel::roleNames() const
{
    ...
    roles[StopID] = "StopID";
    roles[StopName] = "StopName";
    roles[StopDir] = "StopDir";
    return roles;
}
...
void TreeModel::editDeparture(QModelIndex dep, QString newDelay)
{
    ...
    emit dataChanged(index(node->row(), 0, dep.parent()), index(node->row(), 0, dep.parent()));
}

void TreeModel::setProbTime(QModelIndex dep, QString time)
{
    if (!dep.isValid())
        return;
    TreeNode * node =  static_cast<TreeNode*>(dep.internalPointer());
...
void TreeModel::removeProbTime(QModelIndex dep)
{
    if (!dep.isValid())
        return;
    TreeNode * node =  static_cast<TreeNode*>(dep.internalPointer());
...
void TreeModel::edited()
{
    emit someEdits();
}

void TreeModel::calculateTime()
{
    int busses;
    QModelIndex bus;
    int lines;
    ...
    QDateTime depToday = QDateTime::currentDateTime();
    QDateTime depTomorrow = QDateTime::currentDateTime();
    depTomorrow = depTomorrow.addDays(1);
    qint64 msecBtw;

    busses = rowCount(QModelIndex());
    for (int b = 0; b < busses; b++)
    {
        ...
        for (int l = 0; l < lines; l++)
        {
            ...
            {
                dep = index(d, 0, line);
                delay = data(index(d, 1, line)).toString();

                if (!delay.contains("Nie wyruszył"))
                {
                    depTime = data(dep).toString();


                    time = QTime::fromString(depTime, QString("hh:mm"));

                    if (delay.contains("Opóźnienie:"))
                    {
                        delay = delay.split(" ")[1];
                        time = time.addSecs(delay.toInt() * 60);
                    }

                    depToday.setTime(time);

                    if(depToday < now)
                    {
                        msecBtw = now.msecsTo(depTomorrow);

                    }
                    else
                        msecBtw = now.msecsTo(depToday);

                    if (msecBtw >= 86000000)
                        removeProbTime(dep);
                    else
                    {
                        msecBtw = msecBtw / 1000 / 60;
                        setProbTime(dep, QString::number(msecBtw));
                    }
                }
                else
                    removeProbTime(dep);
